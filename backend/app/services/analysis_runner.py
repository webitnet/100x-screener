...
async def _save_batch_results(
    ...
    async with AsyncSessionLocal() as session:
        for pid, modules in by_project.items():
            ...
            # Total = sum of positive scores + penalty
            positive_scores = [s for s in [tok_score, gh_score, oc_score, ca_score, ha_score, sm_score, na_score] if s is not None]
            total = (sum(positive_scores) + penalty) if positive_scores else None
